...
from human_3d_pose_baseline.configs import load_config_h36m as load_config
...
def main():
    ...
    model = get_model(config, num_joints=17)
    ...
    for epoch in range(config.SOLVER.EPOCHS):
        ...
        print(
            f"train_loss: {train_loss:.8f}, test_loss: {test_loss:.8f}, MPJPE: {mpjpe:.4f} mm, MPJPE(best): {mpjpe_lowest:.4f} mm"
        )
