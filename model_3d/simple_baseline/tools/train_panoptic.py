...
from human_3d_pose_baseline.configs import load_config_panoptic as load_config
from human_3d_pose_baseline.datasets import get_dataset_panoptic
from human_3d_pose_baseline.engines import test_epoch_panoptic, train_epoch_panoptic
...
def main():
    ...
    """
    config = load_config()
    panoptic = get_dataset_panoptic(config)
    model = get_model(config, num_joints=15)   
    ...
    for epoch in range(config.SOLVER.EPOCHS):
        ...
        # Training.
        print("Training...")
        train_logs = train_epoch_panoptic(
            config, model, criterion, optimizer, lr_scheduler, panoptic, device
        )

        # Testing.
        print("Testing...")
        test_logs = test_epoch_panoptic(config, model, criterion, panoptic, device)
        ...
        print(
            f"train_loss: {train_loss:.8f}, test_loss: {test_loss:.8f}, MPJPE: {mpjpe*10.0:.4f} mm, MPJPE(best): {mpjpe_lowest*10.0:.4f} mm"
        )
