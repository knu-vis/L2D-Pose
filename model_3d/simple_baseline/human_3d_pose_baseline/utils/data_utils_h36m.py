...
from .camera_utils import project_to_camera
...
def compute_normalization_stats(data, dim, predict_17=True):
    ...
    Args:
        ...
        predict_17 (bool, optional): Whether to use only 17 joints. Defaults to False.
    ...
    if dim == 2:
        ...
        dim_to_ignore = np.where(
            np.array([x in [""] for x in H36M_NAMES])
        )[0]
...
def read_3d_data(actions, data_dir, cams, camera_frame=True, predict_17=True):
    ...
    data_mean, data_std, dim_to_ignore, dim_to_use = compute_normalization_stats(
        train_concat, dim=3, predict_17=predict_17
    )
...
def project_to_cameras(pose_set, cams, ncams=4):
    ...
    for t3dk in sorted(pose_set.keys()):
        ...
        for cam_idx in range(1, ncams + 1):
            R, T, f, c, k, p, name = cams[(subj, cam_idx)]
            pts2d, _, _, _, _ = project_to_camera(t3d, R, T, f, c, k, p)  # (nx32)x2
...
def create_2d_data(actions, data_dir, cams):
    ...
    train_concat = copy.deepcopy(np.vstack(list(train_set.values())))
    data_mean, data_std, dim_to_ignore, dim_to_use = compute_normalization_stats(
        train_concat, dim=2, predict_17=True
    )
