...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
        ...
        for key in range(len(poses_2d)):
            ...
            keys = np.repeat(key, len(bounds - 1))
        ...
        if poses_3d is not None:
            self.batch_3d = np.empty((batch_size, chunk_length, poses_3d[key].shape[-2], poses_3d[key].shape[-1])) # [B,1,17,3]
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        ...
        if self.cameras is not None:
            ...
            if flip:
                self.batch_cam[ 2] *= -1
                self.batch_cam[ 7] *= -1
        action, subject, cam_index = 'hd', 'panoptic', 0
