...
def get_uvd2xyz(uvd, gt_3D, cam):
    ...
    root = gt_3D[:, :, 2, :].unsqueeze(-2).repeat(1, 1, V, 1).clone()# N*T*V*3
    ...
    z_global[:, :, :2] = dec_out_all[:, :, :2, 2] + root[:, :, :2, 2]  # N*T*V
    z_global[:, :, 3:] = dec_out_all[:, :, 3:, 2] + root[:, :, 3:, 2]  # N*T*V
    ...
    xyz_offset = (xyz_global - xyz_global[:, :, 2, :].unsqueeze(-2).repeat(1, 1, V, 1))# N*T*V*3
...
def sym_penalty(dataset,keypoints,pred_out):
    ...
    if dataset == 'h36m':
        if keypoints.startswith('sh'):
            ...
            left_bone = [(2,6),(6,7),(7,8),(0,3),(3,4),(4,5)]
            right_bone = [(2,12),(12,13),(13,14),(0,9),(9,10),(10,11)]
