...
from common.arguments_panoptic import opts as parse_args
from common.utils_panoptic import *
# from common.load_data_hm36 import Fusion
from common.h36m_dataset import Human36mDataset
import time

from common.load_data_panoptic import Fusion
...
def step(split, args, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            test_mpjpe_list.append(test_p1.item() * 1000.0)
    ...
    if split == 'train':
        ...
        return np.mean(test_mpjpe_list), p2
...
if __name__ == '__main__':
    ...
    if args.create_file:
        ...
        shutil.copyfile(src="common/arguments_panoptic.py", dst = os.path.join(args.checkpoint, args.create_time + "_arguments_panoptic.py"))
        shutil.copyfile(src="model/model_IGANet_panoptic.py", dst = os.path.join(args.checkpoint, args.create_time + "_model_IGANet_panoptic.py"))
        shutil.copyfile(src="common/utils_panoptic.py", dst = os.path.join(args.checkpoint, args.create_time + "_utils_panoptic.py"))
    ...
    if args.train:
        train_data = Fusion(opt=args, train=True)
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size,
                                                       shuffle=True, num_workers=int(args.workers), pin_memory=True)
    if args.test:
        test_data = Fusion(opt=args, train=False)
