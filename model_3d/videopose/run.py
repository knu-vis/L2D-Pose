...
dataset_path = 'dataset/data_3d_' + args.dataset + '.npz'
if args.dataset == 'h36m':
    ...
    dataset = CustomDataset('dataset/data_2d_' + args.dataset + '_' + args.keypoints + '.npz')
else:
...
keypoints = np.load('dataset/data_2d_' + args.dataset + '_' + args.keypoints + '.npz', allow_pickle=True)
...
if not args.evaluate:
    ...
    if semi_supervised:
        ...
        if torch.cuda.is_available():
            model_traj = model_traj.to(device)
            model_traj_train = model_traj_train.to(device)
        ...
        if semi_supervised:
            ...
            for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in \
                ...
                if torch.cuda.is_available():
                    ...
                    inputs_2d_semi = inputs_2d_semi.to(device)
