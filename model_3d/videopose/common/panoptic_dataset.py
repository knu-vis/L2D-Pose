...
def group_consecutive_numbers(indices, treshold=81):
    result = []
    temp = []
...
data_dir = '/media/vis/SSD_2TB/3d-guided_2d-HPE/data_3d/panoptic_3d'
        ...
        for i, frame in enumerate(frames):
            if i % interval == 0:
                ...
                for pose in poses:
                    ...
                    seq_data.setdefault(pose['id'], []).append(temp)   
        
        for key, value in seq_data.items():
            pose3d_world = np.array(value).reshape((-1, 3))
            for idx, cam in enumerate(cameras):
                ...
                groudped_valid_indices = group_consecutive_numbers(valid_idx, treshold=81)
                for group_idx in groudped_valid_indices:
                    consecutive_pose2d = pose2d[group_idx].copy()
                    consecutive_pose2d[..., :2] = normalize_screen_coordinates(consecutive_pose2d[..., :2], w=width, h=height)
                    ...
                    data_3d.append(np.array(pose3d_camera)[group_idx] / 100)   # cm to m
                    data_2d.append(consecutive_pose2d)
